...
export function LivrableCrudCombobox({
  listKey,
  value,
  onChange,
  placeholder,
  ...
  const saveEdit = () => {
    ...
    updateLivrableOption(listKey, editingId, { label: next })
    if (value === items.find((i) => i.id === editingId)?.label) onChange(next)
    cancelEdit()
  }
  ...
  const addNew = () => {
    ...
    if (!trimmed) return
    if (items.some((it) => it.label.toLowerCase() === trimmed.toLowerCase())) {
      setNewLabel("")
      onChange(items.find((it) => it.label.toLowerCase() === trimmed.toLowerCase())!.label)
      return
    }
  ...
  const stopItemSelect = (e: React.SyntheticEvent) => {
    // Prevent cmdk from selecting the parent CommandItem
    e.preventDefault()
          ...
          <CommandList>
            ...
            <CommandGroup>
              {items.map((it) => {
                ...
                const isSelected = value === it.label
                return (
                  <CommandItem
                    key={it.id}
                    value={it.label}
                          ...
                          <Button
                            ...
                            onClick={(e) => {
                              ...
                              if (confirm(t("settings.livrableOptions.confirmDelete"))) {
                                deleteLivrableOption(listKey, it.id)
                                if (value === it.label) onChange("")
                              }
                            }}
