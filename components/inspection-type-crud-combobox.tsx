...
export function InspectionTypeCrudCombobox({ value, onChange, placeholder }: Props) {
  const { t } = useLocale()
  const { inspectionOptionLists, addInspectionTypeOption, updateInspectionTypeOption, deleteInspectionTypeOption } =
    useAppStore()
  ...
  const items = inspectionOptionLists.types || []
  const selected = items.find((it) => it.label === value) || null

  const stopItemSelect = (e: React.SyntheticEvent) => {
    e.preventDefault()
    // @ts-expect-error stopPropagation exists on all synthetic events
    e.stopPropagation?.()
  }
  ...
  const saveEdit = () => {
    ...
    if (!next) return
    updateInspectionTypeOption(editingId, { label: next })
    if (selected?.id === editingId) onChange(next)
    cancelEdit()
  }
  ...
  const addNew = () => {
    ...
    const id = `insp-type-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
    addInspectionTypeOption({ id, label: trimmed })
    onChange(trimmed)
    setNewLabel("")
    setOpen(false)
  }
                          ...
                          <Button
                            ...
                            onClick={(e) => {
                              ...
                              if (confirm(t("settings.livrableOptions.confirmDelete"))) {
                                deleteInspectionTypeOption(it.id)
                                if (selected?.id === it.id) onChange("")
                              }
                            }}
