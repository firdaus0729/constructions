...
export function ObservationContributingConditionCrudCombobox({
  value,
  onChange,
  placeholder,
}: {
  ...
  const { t } = useLocale()
  const { observationOptionLists, addObservationContributingConditionOption, updateObservationContributingConditionOption, deleteObservationContributingConditionOption } = useAppStore()
  ...
  const items = observationOptionLists.contributingCondition || []
  ...
  const saveEdit = () => {
    ...
    if (!next) return
    updateObservationContributingConditionOption(editingId, { label: next })
    cancelEdit()
  }
  ...
  const addNew = () => {
    ...
    addObservationContributingConditionOption({ id, label: trimmed })
    onChange(id)
    setNewLabel("")
    setOpen(false)
  }
                          ...
                          <Button
                            ...
                            onClick={(e) => {
                              ...
                              if (confirm(t("settings.livrableOptions.confirmDelete"))) {
                                deleteObservationContributingConditionOption(it.id)
